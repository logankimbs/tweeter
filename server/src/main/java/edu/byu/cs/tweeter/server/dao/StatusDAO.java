...
public class StatusDAO {
    public GetFeedResponse getFeed(GetFeedRequest request) {
        ...
        if(request.getLimit() > 0) {
            if (allStatuses != null) {
                int statusIndex = getStatusStartingIndex(request.getLastItem().getUser().getAlias(), allStatuses);
    ...
    private int getStatusStartingIndex(String alias, List<Status> allStatuses) {
        int statusIndex = 0;
        ...
        if(alias != null) {
            ...
            for (int i = 0; i < allStatuses.size(); i++) {
                if(alias.equals(allStatuses.get(i).getUser().getAlias())) {
